...
class GXAsyncWorkflowPublish
{
     ...
     * @var string|null
     */
    protected $createdBy = null, $createdByName = null;
     ...
     * @var int
     */
    protected $totalStep = 0, $connectionTimeout = 60;

    /**
     * @var GXAsyncWorkflowForm|null
     */
    protected $firstStep = null;
     ...
     */
    protected $AMQPStreamConnection = null;
     ...
     * @throws \Exception
     */
    public function __construct($action = null, $reference = null, $referenceType = null, $isStrict = false)
    {
        $this->action = $action;
        $this->isStrict = $isStrict;

        $this->onReference($reference, $referenceType);
    }
     ...
     * @throws \Exception
     */
    public function onReference($reference, $referenceType = null): GXAsyncWorkflowPublish
    {
        if ($reference instanceof Model) {
            $this->referenceId = $reference->id;
            $this->referenceType = get_class($reference);
    ...
    private function setConnection()
    {
        ...
        try {
            $this->AMQPStreamConnection = new AMQPStreamConnection(
                ...
                null,
                'en_US',
                ($this->connectionTimeout ?: (config('gx-rabbitmq.timeout') ?: 60))
            );
        } catch (\Throwable $throw) {
            $this->logError($throw->getMessage());
        }
